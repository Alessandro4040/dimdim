...
function processarMensagemChat(valor, labelExibicao) {
    // 1. Mostra o que o usuário digitou/clicou
    adicionarBalaoChat('user', labelExibicao);
    mostrarBotoesRapidos([]); // Esconde botões rápidos após a escolha
    
    const etapaAtual = chatFluxo.etapa;
    
    // 2. Lógica de Perguntas em Cascata
    if (etapaAtual === 0) {
        ...
        if (chatFluxo.dadosTemp.tipo === 'transferencia') {
            chatFluxo.etapa = 3.5; // Etapa extra para transferências
            setTimeout(() => {
        ...
        chatFluxo.etapa = 5; // Pula para a etapa de confirmação
        setTimeout(() => {
...
async function finalizarSalvamentoChat() {
    ...
    // Pega a data atual na timezone correta (igual ao seu app.js)
    const tzOffset = (new Date()).getTimezoneOffset() * 60000;
    ...
    try {
        if (chatFluxo.dadosTemp.tipo === 'transferencia') {
            ...
            const transacao = {
                ...
                pago: true, // Registros rápidos pelo chat entram como pagos
                parcela_num: 1,
                parcela_total: 1,
                foto: null,
                sinc: false,
        ...
        scheduleSync(); // Chama a sua rotina de envio pro Google Sheets
        setTimeout(() => fecharModais(), 1500);
        
    } catch (err) {
        console.error(err);
        adicionarBalaoChat('bot', '❌ Puxa, deu um erro ao salvar no banco local.');
    }
}
